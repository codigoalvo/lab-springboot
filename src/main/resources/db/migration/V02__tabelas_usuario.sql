...
CREATE TABLE security_user_profile (
	...
	CONSTRAINT fk_user_sup FOREIGN KEY (id_user) REFERENCES security_user(id),
	CONSTRAINT fk_profile_sup FOREIGN KEY (id_profile) REFERENCES security_profile(id)
);
...
CREATE TABLE security_profile_authority (
	...
	CONSTRAINT pk_profile_authority PRIMARY KEY (id_profile, id_authority),
	CONSTRAINT fk_profile_spa FOREIGN KEY (id_profile) REFERENCES security_profile(id),
	CONSTRAINT fk_authority_spa FOREIGN KEY (id_authority) REFERENCES security_authority(id)
);

INSERT INTO security_user (login, password) VALUES ('usuario@email.com', /* usuario123 */ '$2a$10$vg7c5ezwykIr5lWG7Rly9uGQFNOtdaXrEZMz9s64mP.ivmalKwUuC');
INSERT INTO security_user (login, password) VALUES ('admin@email.com', /* admin123 */ '$2a$10$hHr8kI3BRHpweNJdn0Mg.e83mmZKkbzUf9twUbJ4cKmBZRNdf8QFK');
...
INSERT INTO security_user_profile (id_user, id_profile) VALUES (
	(SELECT id FROM security_user u WHERE u.login = 'usuario@email.com'),
	(SELECT id FROM security_profile p WHERE p.name = 'USU')
);

INSERT INTO security_user_profile (id_user, id_profile) VALUES (
	(SELECT id FROM security_user u WHERE u.login = 'admin@email.com'),
	(SELECT id FROM security_profile p WHERE p.name = 'USU')
);
